...
public class MumlaService extends HumlaService implements
    ...
    // --- Falconclaw: Bluetooth media-button push-to-talk ---
    // Captures the AVRCP media button (play/pause/stop) sent by the Falconclaw
    // PTT and uses it to toggle Mumble talk state. Active only while connected.
    private MediaSession mMediaButtonSession;

    private void enableMediaButtonPtt() {
        ...
        try {
            ...
            mMediaButtonSession.setCallback(new MediaSession.Callback() {
                ...
                public boolean onMediaButtonEvent(Intent mediaButtonIntent) {
                    ...
                    Log.d(TAG, "FCPTT media button: keycode=" + ke.getKeyCode()
                            + " action=" + ke.getAction() + " repeat=" + ke.getRepeatCount());
                    int code = ke.getKeyCode();
                    boolean isPtt = code == KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE
            ...
            PlaybackState ps = new PlaybackState.Builder()
                    ...
                    .build();
            mMediaButtonSession.setPlaybackState(ps);
            mMediaButtonSession.setActive(true);
            Log.i(TAG, "Falconclaw media-button PTT enabled");
        } catch (Exception e) {
            Log.e(TAG, "Failed to enable media-button PTT", e);
        }
    }

    private void disableMediaButtonPtt() {
        if (mMediaButtonSession != null) {
            try {
                mMediaButtonSession.setActive(false);
                mMediaButtonSession.release();
            } catch (Exception e) {
                Log.e(TAG, "Error releasing media-button session", e);
            }
